...
import numpy as np
from Functions import store_vars
...
def process(file):
    ...
    text = None   # free text memory after use
    ...
    """
    X = np.zeros((len(sections), len_per_section, char_size), dtype=np.int32)
    y = np.zeros((len(sections), char_size), dtype=np.int32)
    ...
    with graph.as_default():
        ...
        for i in range(len_per_section):
            ...
            """
                Concatenates data and output to be used to calculate logits, loss and optimizer functions.
    
                outputs_all_i   :   
                labels_all_i    :   
    
                concat axis=0, concatenates along the 0th dimension
            """
        ...
        w = tf.Variable(tf.truncated_normal([hidden_nodes, char_size], -0.1, 0.1))  # weight for logits
        b = tf.Variable(tf.zeros([char_size]))  # bias for logits
